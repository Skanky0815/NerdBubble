...
class AssmodeeProvider implements Provider
{
    ...
    public function loadArticles(): Collection
    {
        ...
        $mapToArticle = fn (array $articleData): Article => AsmodeeArticle::create($articleData);
        $filter = fn (Article $articleDto): bool => $this->keywordFilter->matchKeyword($articleDto->filterText);

        return collect($response['articles'])->map($mapToArticle)->filter($filter);
    }
